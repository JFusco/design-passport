...
The production manifest keeps the existing organization-published Figma plugin ID. A second `manifest.dev.json` identity is named **Design Passport (Development)** and always shows a persistent warning. The default build embeds a Development channel; `pnpm build:release` embeds Production. Plugin version `0.4.0`, ruleset `1.0.0-beta.4`, the Git build SHA, and channel appear in bootstrap state, the footer, report-v3 data, exports, and certificate-v2 data.
