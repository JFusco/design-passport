...
- Separately, import `manifest.dev.json` when smoke-testing an unreleased local build; confirm **Design Passport (Development)** launches from **Plugins → Development**, shows its persistent warning, uses a distinct plugin ID, and identifies the Development channel. Never import the production manifest for local development.
