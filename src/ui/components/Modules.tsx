    ...
    <section className="panel stack">
      ...
      {pages.map((page) => (
        <details className="page-breakdown" key={page.pageId} open={pages.length === 1}>
          <summary><span><strong>{page.pageName}</strong><small>{page.modules.length} module{page.modules.length === 1 ? "" : "s"} · {page.actionableCount} actionable issues · {page.occurrenceCount} occurrences</small></span><span className="grade-summary">{Object.entries(page.gradeCounts).filter(([, count]) => count > 0).map(([grade, count]) => `${count}${grade}`).join(" · ")}</span></summary>
          <div className="module-list">
            {page.modules.map((module) => (
              <article className="module-row" key={module.rootId}>
                ...
                <div className="module-metrics"><span><strong>{module.actionableCount}</strong> actionable issues</span><span>{module.occurrenceCount} affected occurrences</span><span>Occurrences: {module.failCount} fail · {module.reviewCount} review</span></div>
                <div className="module-diagnostics">{module.relatedGroupCount > 0 ? <small>{module.relatedGroupCount} related group{module.relatedGroupCount === 1 ? " needs" : "s need"} individual review; a shared fix is unverified.</small> : null}<small>Weakest: {module.weakestAxes.map((axis) => AXIS_LABELS[axis]).join(" · ")}</small>{module.topRules.length > 0 && <small>Top rules by occurrence: {module.topRules.map((rule) => `${rule.ruleId} (${rule.count})`).join(" · ")}</small>}</div>
                    ...
                    <div className="variant-list">
                      {module.variants.map((variant) => (
                        <button key={variant.variantId} onClick={() => props.onViewVariantFindings(module.rootId, variant.variantId)}>
                          ...
                          <span>{variant.nodeCount} nodes · {variant.actionableCount} actionable issues · {variant.occurrenceCount} occurrences</span>
                        </button>
                      ))}
                    </div>
                  </details>
