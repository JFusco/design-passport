...
export function ProfileEditor(props: ProfileEditorProps) {
  ...
  return (
    <section className="panel stack profile-form">
      ...
      <fieldset><legend>Pipeline requirements</legend><label className="collection-row"><input type="checkbox" checked={props.profile.requireCodeConnect === true} onChange={(event) => props.onChange({ ...cloneProfile(props.profile), requireCodeConnect: event.target.checked })} /><span><strong>Require Code Connect</strong><small>Leave off when MCP/API consumers use native Figma component and variable data without code mappings.</small></span></label></fieldset>
