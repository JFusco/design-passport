...
GitHub reconciliation uses the canonical `Sync context wiki` and `Sync wiki issue state` Actions, reviewable `bot/wiki-*` branches, Node 24.14.0, and a `PR_BOT_TOKEN` secret with contents and pull-request write access. Merge sync is event-driven with manual single-PR replay, optional `since` batch replay, and a 90-day default audit. Issue state runs Mondays at `30 11 * * 1`, refreshes cited issues, and performs the same missed-merge audit. Portable jobs invoke Node scripts directly, compare proposed content before committing, and use REST pull-request updates. Wiki checks reject duplicate fields and malformed bracket lists across every Markdown page under `wiki/`. Both writers set `GRAPHIFY_SKIP_HOOK=1`; developer Git events retain native Graphify behavior.
