...
`src/companion/main.ts` builds to `dist/companion.mjs`. It is the only networked part of the design. It validates exact Figma URLs, keeps `FIGMA_TOKEN` local, ingests each source into an isolated graph, produces sanitized packs, imports explicitly exported learning envelopes, groups exact observation keys, and generates candidate wording.

The Next.js application under `apps/companion/` serves the maintainer interface only on `127.0.0.1`. The CLI creates a capability for each process. The bootstrap route exchanges that capability for an HttpOnly, SameSite cookie and removes it from the browser URL. Exact Host checks, same-origin writes, and route-level authorization protect every read and mutation. Repository reads have no write side effects. Imports, draft edits, decisions, and rebuilds use one workspace lock and atomic private file replacement.
...
The production manifest keeps the existing organization-published Figma plugin ID. A second `development/manifest.json` identity is named **Design Passport (Development)** and always shows a persistent warning. Its separate directory avoids Figma's one-development-plugin-per-manifest-directory deduplication. The default build embeds a Development channel and mirrors the exact generated UI/controller bytes into ignored `development/dist/` output because Figma confines a development plugin to its manifest directory. `pnpm build:release` embeds Production only in root `dist/` and never overwrites the development copy. Plugin version `0.4.0`, ruleset `1.0.0-beta.4`, the Git build SHA, and channel appear in bootstrap state, the footer, report-v3 data, exports, and certificate-v2 data.

No runtime network access or version service is introduced. Freshness is established by the identity embedded in the published bundle and the single current organization plugin record. Production builds require a clean checkout; Development identities append a content-derived dirty suffix. A build metadata sidecar hashes the generated controller and UI, and the Development copy step accepts only matching Development-stamped artifacts. Historical results are preserved with their original producer identity instead of being silently upgraded.
